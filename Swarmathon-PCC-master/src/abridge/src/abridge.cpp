...
void cmdHandler(const geometry_msgs::Twist::ConstPtr& message) {
    ...
    if (linearSpeed != 0. && turnSpeed != 0.) {
        if (turnSpeed >= 0){ // rotate left
            arcRadius = (linearSpeed*1.5 / turnSpeed*8);
            // right speed is linear speed
            rightSpeed = (arcRadius + ((100*wheelBase)/2.)) * turnSpeed * 8;
            // left speed is r * turnSpeed
            leftSpeed = (arcRadius - ((100*wheelBase)/2.)) * turnSpeed * 8; // *8 for radiant 
            leftSpeed /= 1.5;
        } else{
            turnSpeed *= -1;
            arcRadius = (linearSpeed*1.5 / turnSpeed*8);
            // right speed is linear speed
            leftSpeed = (arcRadius + ((100*wheelBase)/2.)) * turnSpeed * 8;
            // left speed is r * turnSpeed
            rightSpeed = (arcRadius - ((100*wheelBase)/2.)) * turnSpeed * 8; // *8 for radiant
            rightSpeed /= 1.5;
        }
        sprintf(moveCmd, "a,%d:%d;\n", (int)(leftSpeed * 255), (int)(rightSpeed * 255) ); // the format is a,255:255; int seperated by : and ends with ;
